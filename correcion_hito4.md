Necesitamos hacer dos ajustes vitales en tu archivo src/data/websocket_client.py:

Heartbeat (Latido): Decirle a aiohttp que envíe un "Ping" invisible cada 30 segundos para engañar al firewall del broker y mantener el túnel abierto aunque el mercado esté callado.

Reconexión Infinita: Envolver la conexión en un bloque while True para que, si Upstox reinicia sus servidores o se cae el internet por un microsegundo, tu bot se levante solo sin que tú tengas que ir al servidor a reiniciar el contenedor Docker.

Reemplaza tu método connect_and_listen dentro de src/data/websocket_client.py con esta versión robusta:
