El error está en cómo le dijimos a Pandas que buscara. En la bolsa india (NSE), y específicamente en la base de datos de Upstox:

La columna instrument_type guarda el valor "OPTIDX" (Options Index).

La columna option_type es la que guarda "CE" (Call) y "PE" (Put).

En nuestro código anterior estábamos buscando 'CE' y 'PE' en la columna equivocada (instrument_type), por lo que Pandas filtró todo y devolvió una lista vacía.

La Solución Rápida
Vamos a actualizar tu archivo src/data/instrument_mapper.py para apuntar a la columna correcta (option_type).

Reemplaza todo el contenido de src/data/instrument_mapper.py por este:
...
class InstrumentMapper:
    ...
    def get_options_for_underlying(self, underlying_symbol: str, expiry_date: str = None):
        ...
        return options_df[['instrument_key', 'tradingsymbol', 'strike', 'option_type', 'expir